...
module.exports={
    ...
    EncontrarPujaProducto: async function (req, res, next) {
        try {
            let id = req.query.idProd;
            if (!id) {
                // Valida si el parámetro está presente
                return res.status(400).send({ codigo: 1, mensaje: "Falta el ID del producto." });
            }
    
            let idProducto = new BSON.ObjectId(id);
    
            await clienteMongoDB.connect();               
            try {
                const db = clienteMongoDB.db(process.env.DB_MONGODB);
                const pujas = await db.collection("pujas").findOne({ idProd: idProducto });
    
                if (!pujas) {
                    // Si no se encuentra la puja, envía respuesta y detén la ejecución
                    console.log("No se encontro el producto con esa puja")
                    return res
                        .status(500)
                        .send({ codigo: 1, mensaje: "No hay puja de ese producto amor" });
                }
    
                // Si se encuentra la puja, envía la respuesta
                console.log("encontrado el producto con la puja")
                return res
                    .status(200)
                    .send({ codigo: 0, mensaje: "Existe puja de ese producto amor", data:{pujas} });
            } catch (error) {
                console.log("Error al conectarte a la base de datos ", error);
                return res
                    .status(500)
                    .send({ codigo: 1, mensaje: "Error interno del servidor" });
            }
        } catch (error) {
            console.log("Error en encontrar puja servidor...", error.message);
            return res
                .status(500)
                .send({ codigo: 1, mensaje: "Error interno al procesar la solicitud" });
        }
    },
    OpcionPujar:async function (req,res,next) {
        try{
            const { idProducto,precioPuja } = req.body;

            const id = new BSON.ObjectId(idProducto);
            if (!id) {              
                return res.status(400).send({ codigo: 1, mensaje: "No esta el id" });
            }
            ...
            try {
                await _sesionTransaccion.withTransaction(async () => {
                    const db = clienteMongoDB.db(process.env.DB_MONGODB);
                    const pujas = await db.collection("pujas").findOne({ idProd: id });
                    if (!pujas) {
                        console.log("No se encontro el producto con esa puja")
                        return res
                            .status(500)
                            .send({ codigo: 1, mensaje: "No hay puja de ese producto" });
                    }
                    const pujasid=new BSON.ObjectId(pujas._id);
                    const resultadoPujar = await db.collection('pujas').updateOne(
                        { _id: pujasid},
                        { $set: { pujaMasAlta: precioPuja } }
                    );

                    if(!resultadoPujar){
                        console.log("No se puedo hacer la puja")
                        return res
                            .status(500)
                            .send({ codigo: 1, mensaje: "No se puedo hacer la puja" });
                    }

                    return  res.status(200).send({
                        codig:0,
                        mensaje: "Puja efectuada correctamente"
                    });
                })
            }catch(error){
                console.log("Error al conectarte a la base de datos ", error);
            ...
            console.log("Error en la opcion de pujar", error);
        }
    }
