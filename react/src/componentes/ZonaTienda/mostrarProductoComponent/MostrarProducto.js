...
function MostrarProducto(){
    ...
    async function HandlerBotonComprarYaClick(){
        ...
        setPedido({comprarYa:{producto:producto,cantidad:1}});   
        try{
            let respuesta=await restTienda.DevolverDireccion(email);
            if(respuesta.codigo==0){
                navigate('/Tienda/ComprarYa');
            }else{
                navigate('/Tienda/Direccion');
            }
        }catch(error){
            console.log("ERROR EN RESPUESTA de coger direccion cliente", error);
        }                                  
    };
    ...
    return (
        <div className="container mt-5">
            ...
            <div className="col-md-7">
                ...
                {producto.precio?.$numberDecimal ? `${producto.precio.$numberDecimal} EUR` : `${producto.precio} EUR`}
                </h4>
                <p>Estado: <strong>{producto.estado}</strong></p>
                <div className="d-grid gap-2 my-3">
                    {/*antes en el onclick ->  () => navigate(`/Tienda/ComprarYa/${producto._id}`) pero esta mal*/}
