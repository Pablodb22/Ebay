...
function Header( { categorias } ) {
   ...
   /// console.log('accestoken' ,accessToken); <-- aparece primero vacio y cuando inicia sesion el usuario y vuelve al header aparece el accestoken del usuario
    ...
    useEffect(
        () => {
            // Si 'categoriaSelec' tiene un valor y no se encuentran las subcategorías en caché, 
            // entonces hacemos la solicitud para obtener las subcategorías de esa categoría.
            if (categoriaSelec && !subCategoriasCache[categoriaSelec]) {
                ...
                .then(subcategorias => {
                    console.log('Subcategorías recuperadas...', subcategorias);
                    setSubCategoriasCache({
                        ...subCategoriasCache,
                        [categoriaSelec]: subcategorias
                    });
                })
                .catch(error => console.log('Error al obtener subcategorías...', error));
            }
        },
        [categoriaSelec, subCategoriasCache]  // Dependencias del useEffect
    );
                        ...
                        <div>
                            <ul className="navbar-nav ms-auto mb-2 mb-lg-0">
                                ...
                                {cliente ? (
                                                    ...
                                                    <div className="card mb-3" style={{ maxWidth: '250px' }}>
                                                        <div className="row g-0">
                                                            <div className="col-md-4">
                                                                <img src={cliente.imagenAvatar} className="img-fluid rounded-start" alt="Avatar" />
                                                            </div>
                                                            <div className="col-md-8">
                                                                <div className="card-body">
                                                                    <h6 className="card-title">{cliente.nombre} {cliente.apellidos}</h6>
                                                                    <p className="card-text">
                                                                        <Link to='/Cliente/Panel/Inicio'>Configuración de la cuenta</Link>
                                                                        <br />
                                        ...
                                        <NavLink className="nav-link" to='/Cliente/Login'>Inicio Sesión</NavLink>
                                    )}
                                </li>
                                    ...
                                    categorias && categorias.map(
                                        el=>{
                                            return <li className="nav-item"
                                                         key={el._id}
                                                        onMouseEnter={ ()=> setCategoriaSelect( el.pathCategoria) }
