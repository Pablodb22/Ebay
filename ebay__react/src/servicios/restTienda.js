...
let restTienda={
    ...
    EncontrarPuja: async function (id) {
        try{     
        console.log("id en servicio",id )   
        let respEncontrarPuja=await fetch(`http://localhost:3003/api/zonaTienda/EncontrarPujaProducto?idProd=${id}`,{ method: 'GET'});
        return respEncontrarPuja.json();
        }catch(error){
            console.log('error al encontrar puja en servicio', error);
            return null;
        }
    },
    Pujar: async function (idProd,puja) {
        try{
            let respHacerPuja=await fetch('http://localhost:3003/api/zonaTienda/OpcionPujar',
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            return respHacerPuja.json();
        }catch(error){
            console.log('error al hacer puja en servicio', error);
            return null;
        }
    }
}
