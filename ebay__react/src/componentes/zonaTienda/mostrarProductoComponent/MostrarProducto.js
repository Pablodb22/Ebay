...
function MostrarProducto() {
    ...
    const setPrecioMasAlto = useGlobaStore(state => state.setPrecioMasAlto);
    const setProducto = useGlobaStore(state => state.setProducto); // Obtener la función para establecer el producto
    const [puja, setPuja] = useState(false);
    ...
    async function HandlePujarClick() {
        const respuestaServer = await restTienda.EncontrarPuja(producto._id);
        if (respuestaServer.codigo === 0) {
            setPuja(true);
            console.log('*****',respuestaServer.data.pujas.pujaMasAlta)
            setPrecioMasAlto(respuestaServer.data.pujas.pujaMasAlta);
            setProducto(producto); // Almacenar el producto en el estado global
            navigate('/Tienda/Puja'); // Navegar a la página de puja
        } else {
            setPuja(false);
            console.log('No se pudo encontrar la puja.');
        }
    }

    useEffect(() => {
        const saberPujas = async () => {
            try {
                const respuestaServer = await restTienda.EncontrarPuja(producto._id);
                console.log("Respuesta del servidor...", respuestaServer);
                if (respuestaServer.codigo == 0) {
                    setPuja(true);                    
                    setPrecioMasAlto(respuestaServer.data.pujas.pujaMasAlta);
                } else {
        ...
        <div className="container mt-5">
            <div className="row">
                ...
                <div className="col-md-7">
                    ...
                    <div className="d-grid gap-2 my-3">
                        ...
                        {puja && (
                            <button className="btn btn-warning btn-lg" type="button" onClick={HandlePujarClick}>
                                Puja
                            </button>
                        )}
            ...
            <div className="mt-5">
                ...
                <div className="row">
                    ...
                    <div className="col-md-3">
                        <div className="card">
                            <img src="https://via.placeholder.com/150 ```javascript
" className="card-img-top" alt="Related Product" />
                            <div className="card-body">
